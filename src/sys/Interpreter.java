...
enum Fct {
	LIT,   
	OPR,  //过程调用   二元比较符
	LOD,  //将层差为L、偏移量为A的存储单元的取值到栈顶，t+1
	STO,  //存取指令
	CAL,  //过程调用指令
	INT,  //存取指令
	JMP, //转移指令  无条件
 ...
 */
class Instruction { //p-code的指令格式是  F L A
	/**
	 * 虚拟机代码指令
	 */
	public Fct f;//指令的操作码
		...
		do {
			...
			case OPR:				// 数学、逻辑运算
				...
				case 0:           //结束被调用过程，返回调用点并退栈
					...
					b = s[t+1];
					break; 
				case 1:        //求栈顶元素的相反数，结果值留在栈顶
					s[t-1] = -s[t-1];
					break;
				case 2:       //次栈顶与栈顶元素相加，结果存在次栈顶，t减1
					t--;
					s[t-1] = s[t-1]+s[t];
					break;
				case 3:    //次栈顶的值减去栈顶元素，结果存在次栈顶，t减1
					t--;
					s[t-1] = s[t-1]-s[t];
					break;
				case 4:
				...
				case 16:
					...
					t++;
					break;
				case 17:  //次栈顶与栈顶 or运算 结果保留在此栈顶，t--;
					t--;
					boolean s_1=false;
					boolean s_2=false;
					if(s[t-1]>0)
				...
				case 18:
					...
					if(s[t]>0)
						s_4=true;
					s[t-1]=(s_3&s_4)==true?1:0;
					
				case 19:
					t--;
					s[t-1]=s[t-1]>0?0:1;   //栈顶>0 . 则把栈顶置为0
