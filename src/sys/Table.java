...
public class Table {
	...
	public class Item {
		...
		int varkind;       //变量类型 0表示integer  , 1表示boolean, 2表示其他
		int val;			// 数值，仅const使用
	 ...
	 * @param k   该符号的类型：const, var, procedure
	 * @param lev 名字所在的层次
	 * @param varkind  变量类型  0-integer 、1-Boolean、2-other
	 */
	public void enter(Objekt k,int lev,int dx,int varkind) {
		tx ++;
		Item item = get(tx);
		item.name = PL0.lex.id;			// 注意id和num都是从词法分析器获得
		item.kind = k;
